...
(define-public rustdesk
  (package
    ...
    (inputs (list (list "glibc" glibc)
                  (list "gtk+" gtk+)
                  (list "pango" pango)
                  (list "at-spi2-core" at-spi2-core)
                  (list "cairo" cairo)
                  (list "gdk-pixbuf" gdk-pixbuf)
                  (list "glib" glib)
                  (list "wayland" wayland)
                  (list "pulseaudio" pulseaudio)
                  (list "libxtst" libxtst)
                  (list "libxcb" libxcb)
                  (list "libx11" libx11)
                  (list "libxfixes" libxfixes)
          ...
          (add-after 'patch-elf 'create-wrapper
            (lambda* (#:key inputs outputs #:allow-other-keys)
              (let* ((out (assoc-ref outputs "out"))
                     ...
                     (libpath (string-join (list (string-append libc "/lib")
