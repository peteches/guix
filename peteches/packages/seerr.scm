                ...
                (let ((matches (find-files "." "^package\\.json$")))
                  (match matches
                    ((package-json)
                     ...
                     (format #t "changed directory to '~a'~%"
                             (getcwd)))
                    (() (error "could not find package.json after unpack"))
                    (_ (error "found multiple package.json files" matches)))))))
            ...
            (lambda _
              (let ((configs (filter file-exists?
                ...
                (for-each (lambda (file)
                            (format #t
                             "patching Next.js typecheck config in ~a~%" file)
                            (substitute* file
                              (("typescript:[[:space:]]*\\{[^}]*\\}")
            ...
            (lambda _
              (let ((configs (filter file-exists?
                                     '("next.config.js" "next.config.mjs"
                                       "next.config.ts"))))
                (for-each (lambda (file)
                            (format #t "adding webpack SVG/SVGR rule to ~a~%"
                                    file)
                            (substitute* file
          ...
          (add-after 'add-webpack-svgr-config 'link-node-inputs
            (lambda* (#:key inputs #:allow-other-keys)
                ...
                (let ((root (string-append input "/lib/node_modules")))
                  (if (file-exists? root)
                      (filter (lambda (dir)
                                ...
                                (let* ((rel (substring dir
                                                       (1+ (string-length root))))
                                       (parts (filter (lambda (s)
                                                        (not (string-null? s)))
                                                      (string-split rel #\/))))
                                  (or (= (length parts) 1)
                                      (and (= (length parts) 2)
              ...
              (define (read-package-name package-json)
                ...
                ;; handled correctly.
                (let* ((port (open-pipe* OPEN_READ node-bin "-e"
                                         (string-append
                    ...
                    (let ((target (string-append "node_modules/" module-name)))
                      ...
                      (unless (file-exists? target)
                        (if (writable-node-package? module-name)
                            (begin
                              (format #t
                               "copying writable node module ~a -> ~a~%"
              ...
              (let* ((module-dir "node_modules/wink-jaro-distance")
                ...
                (when (file-exists? module-dir)
                  (call-with-output-file types-file
                    (lambda (port)
                      (display "declare namespace jaro {
  interface Result {
    similarity: number;
    distance?: number;
  }
...
"
                  ...
                  (call-with-output-file (string-append module-dir
                                                        "/orderBy.d.ts")
                    (lambda (port)
                      (display "import { orderBy } from './index';
export = orderBy;
" port)))
...
"
              ...
              (let* ((module-dir "node_modules/humanize-duration")
                ...
                (when (file-exists? module-dir)
                  (call-with-output-file types-file
                    (lambda (port)
                      (display "declare function humanizeDuration(
  duration: number,
  options?: Record<string, unknown>
): string;
...
"
              ...
              (let* ((module-dir "node_modules/web-push")
                ...
                (when (file-exists? module-dir)
                  (call-with-output-file types-file
                    (lambda (port)
                      (display "export function setVapidDetails(
  subject: string,
  publicKey: string,
  privateKey: string
): void;
...
" port)))
          ...
          (add-after 'add-local-type-declarations 'fix-direct-node-module-links
            (lambda* (#:key inputs #:allow-other-keys)
              ...
              ;; processing.
              (let* ((input (or (assoc-ref inputs "node-postcss")
              ...
              ;; node-cache.
              (let* ((input (or (assoc-ref inputs "node-node-cache")
                ...
                (unless input
                  (error
                   "could not find node-cache input; check the input label"))
                (force-link "node_modules/node-cache" source))

              ;; @types/node.
              (let* ((input (or (assoc-ref inputs "node-types-node")
                ...
                (unless input
                  (error
                   "could not find @types/node input; check the input label"))
                (force-link "node_modules/@types/node" source))

              ;; @types/react.
              (let* ((input (or (assoc-ref inputs "node-types-react")
              ...
              ;; zod.
              (let* ((input (or (assoc-ref inputs "node-zod")
                                (assoc-ref inputs "node-zod-4.3.6")))
                     (source (and input
                                  (string-append input "/lib/node_modules/zod"))))
                (unless input
                  (error "could not find zod input; check the input label"))
                (force-link "node_modules/zod" source))

              ;; @tailwindcss/forms.
              (let* ((input (or (assoc-ref inputs "node-tailwindcss-forms")
                                (assoc-ref inputs
                                           "node-tailwindcss-forms-0.5.10")
                                (assoc-ref inputs
                                           "node-tailwindcss-forms-0.5.11")))
                     (source (and input
                                  (string-append input
              ...
              (let* ((input (or (assoc-ref inputs
                                           "node-tailwindcss-typography")
                                (assoc-ref inputs
                                 "node-tailwindcss-typography-0.5.16")))
                     (source (and input
                                  (string-append input
              ...
              (let* ((input (or (assoc-ref inputs "node-dayjs")
...
with ~a~%"
          ...
          (add-after 'fix-direct-node-module-links 'link-node-binaries
            (lambda* (#:key inputs #:allow-other-keys)
              ...
              (for-each (lambda (input)
                          (when (file-exists? (string-append input
                                               "/lib/node_modules"))
          ...
          (replace 'build
            (lambda _
              ...
              (invoke "node" "node_modules/next/dist/bin/next" "build"
                      "--webpack")
