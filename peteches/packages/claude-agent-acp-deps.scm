;;; peteches/packages/claude-agent-acp-deps.scm — node dependency closure
;;; for node-agentclientprotocol-claude-agent-acp-0.42.0, split out of
;;; claude-agent-acp.scm to keep that file to just the culminating product
;;; package. All 49 bindings here are define-public and therefore
;;; auto-exported; nothing enumerates them.

(define-module (peteches packages claude-agent-acp-deps)
...
(define-public node-object-assign-4.1.1
  (package
    ...
    (synopsis "ES2015 @code{Object.assign()} ponyfill")
    (description "ES2015 @code{Object.assign()} ponyfill")
  ...
  (package
    ...
    (arguments
     (list
      ...
      #~(modify-phases %standard-phases
          (delete 'build)
          ;; Upstream's lifecycle scripts are not runnable here and
          ;; make dependent packages fail to install this one.
          (add-before 'repack 'disable-lifecycle-scripts
            (lambda _
              (modify-json (delete-fields '("scripts.prepare"
                                            "scripts.postinstall")
          ...
          (add-after 'patch-dependencies 'delete-dev-dependencies
            (lambda _
              (modify-json (delete-dependencies '("after" "eslint" "express"
                                                  "mocha" "nyc" "supertest"))))))))
    (inputs (list node-vary-1.1.2 node-object-assign-4.1.1))
  ...
  (package
    ...
    (arguments
     (list
      ...
      #~(modify-phases %standard-phases
          (delete 'build)
          ;; Upstream's `prepare' script runs `ts-scripts install', which
          ;; is not available here and makes every dependent package fail
          ;; to install this one.  Matches the copy in seerr-deps.scm.
          (add-before 'repack 'disable-lifecycle-scripts
            (lambda _
              (modify-json (delete-fields '("scripts.prepare"
                                            "scripts.postinstall")
  ...
  (package
    ...
    (arguments
     (list
      ...
      #~(modify-phases %standard-phases
          (delete 'build)
          ;; npm honours "overrides"/"resolutions" and would fetch the
          ;; pinned versions from the registry, which fails offline.  Each
          ;; dependency is already pinned to a store path by Guix, so the
          ;; fields are redundant here.
