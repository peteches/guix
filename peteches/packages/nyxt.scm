    ...
    (package
      ...
      (source
       (origin
         ...
         (sha256
          (base32 "1c58wk113dry0z6kyl1kckj7il314jlyzfppf9x0nxxrx202f9bs")))))))
        ...
        #~(modify-phases #$phases
            (add-after 'unpack 'use-webkit-evaluate-javascript-api
              (lambda _
                ...
                (define p1
                  (str-sub content "(or null (function (condition))) string))"
...
"
                            "       (cffi:make-pointer callback-counter))))")
                           (string-append "  (%nyxt4-webkit-eval-js\n"
...
"
                ...
                (define new-bindings
                  (string-append "; WebKitGTK 2.40+ evaluate_javascript API
"
...
"
                   "\n"))
                (define p3
                  (str-sub p2 "(export 'webkit-web-view-evaluate-javascript)
"
                           (string-append
     ...
     `(#:modules ((guix build node-build-system)
       ...
       #:phases (modify-phases %standard-phases
                  (add-after 'patch-dependencies 'delete-dependencies
                    (lambda args
                      (modify-json (delete-dependencies '("node-gyp"))
...
(define-public (node-package->electron-package node-package electron-headers)
  ...
  (package
    ...
    (name (string-append "electron-"
                         (package-name node-package)))
    (native-inputs (cons (list "electron-headers" electron-headers)
                         (package-native-inputs node-package)))
    (inputs (map (lambda (i)
                   (let ((iname (car i))
                         (ipkg (car (cdr i))))
                     (if (eq? 'node
                              (build-system-name (package-build-system ipkg)))
                         (list iname
                               (node-package->electron-package ipkg
                                electron-headers)) i)))
            ...
            (replace 'configure
              (lambda* (#:key outputs inputs #:allow-other-keys)
                (let ((npm (string-append (assoc-ref inputs "node") "/bin/npm"))
                      (electron-node-headers (string-append (assoc-ref inputs
                                                             "electron-headers")
                                                            "/node_headers")))
                  (invoke npm
                          "--offline"
            ...
            (add-after 'unpack 'patch-package
              (lambda _
                (substitute* "source/core.lisp"
                  (("\"npm\" \"run\" \"start\" \"--\"")
                   (string-append "\""
                                  #$(this-package-input "electron")
                                  "/bin/electron" "\"")))))
            (add-after 'unpack 'add-node-modules
              (lambda _
                (mkdir-p (string-append #$output "/node_modules"))
                (symlink (string-append #$(this-package-input
                                           "electron-node-synchronous-socket")
                          "/lib/node_modules/synchronous-socket")
                         (string-append #$output
                                        "/node_modules/synchronous-socket")))))))
      (native-inputs (list pkg-config sbcl-spinneret sbcl-lisp-unit2
                           sbcl-parenscript))
      (inputs (list electron-36
...
(define %nyxt4-source
  (origin
    ...
    (sha256 (base32 "08zqr1c91l5qzpzhli32lvam254lwsfbjrcxcm6a71plgdp0wvz2"))))
...
(define-public sbcl-nyxt-electron
  (package
    ...
    (inputs (append %nyxt4-common-inputs
                    (list sbcl-cl-electron)))
    (synopsis "Nyxt 4.0.0 (Electron renderer) — SBCL library")
    (description
     "Nyxt [nýkst] is a keyboard-driven web browser designed for
...
"
    ...
    (inputs (append %nyxt4-common-inputs
                    (list gtk+ glib-networking webkitgtk-for-gtk3
            ...
            (add-before 'strip 'make-executable
              (lambda _
                (invoke "sbcl"
                 ...
                 "(require \"asdf\")"
                 "--eval"
                 "(asdf:load-asd (merge-pathnames \"libraries/nasdf/nasdf.asd\"))"
                 "--eval"
                 "(asdf:load-asd (merge-pathnames \"nyxt.asd\"))"
                 "--eval"
                 #$(string-append "(asdf:load-system " system-kw ")")
                 "--eval"
                 #$(string-append "(asdf:make " system-kw ")"))
                (mkdir-p (string-append #$output "/bin"))
                (copy-file "nyxt"
                           (string-append #$output "/bin/nyxt"))))
            (add-after 'make-executable 'add-assets
              (lambda _
                (define (asset name)
                  (string-append #$output "/share/common-lisp/sbcl/"
                                 #$binary-name "/assets/" name))
                (let ((nyxt.desktop (string-append #$output
                  ...
                  (substitute* nyxt.desktop
                    (("^Exec=nyxt")
                     (string-append "Exec="
                                    #$output "/bin/nyxt"))))
                (let ((nyxt.metainfo.xml (string-append #$output
                ...
                (for-each (lambda (size)
                            (let ((out (format #f
                                        "~a/share/icons/hicolor/~dx~d/nyxt.png"
                                        #$output size size)))
                              (mkdir-p (dirname out))
                              (symlink (asset (format #f
                                                      "/icons/nyxt_~dx~d.png"
                                                      size size)) out)))
                          '(16 32 128 256 512))))))))))

(define-public nyxt-webkit
  (let ((base (%make-nyxt-binary sbcl-nyxt-webkit ":nyxt/gi-gtk-application"
                                 "nyxt-webkit")))
    (package
      (inherit base)
      (arguments
...
(define-public nyxt-electron
  (%make-nyxt-binary sbcl-nyxt-electron ":nyxt/electron-application"
                     "nyxt-electron"))
