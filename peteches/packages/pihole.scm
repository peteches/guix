...
(define-public pihole-ftl
  (let* ((version "6.6.2")
    ...
    (package
      ...
      (description
       "pihole-FTL is the core daemon of Pi-hole.  It is a
            ...
            (lambda* (#:key outputs #:allow-other-keys)
              (let* ((out (assoc-ref outputs "out"))
                ...
                (for-each (lambda (f)
                            (substitute* f
                              (("PI_HOLE_SCRIPT_DIR=\"/opt/pihole\"")
                               (string-append "PI_HOLE_SCRIPT_DIR=\""
                                              scripts-dir "\""))))
                ...
                (substitute* (string-append scripts-dir "/utils.sh")
                  ...
                  (("\\$\\{FTL_PID_FILE\\}" all)
                   (if (string=? all "${FTL_PID_FILE}") "${_ftl_pid_file}" all)))
                  ...
                  (substitute* gravity-sh
                    ...
                    (("\"/etc/.pihole/advanced/Scripts/database_migration/gravity-db.sh\"")
                     (string-append "\"" scripts-dir
                                    "/database_migration/gravity-db.sh\""))
                ...
                (let* ((ncurses-bin #$(file-append ncurses "/bin"))
                  ...
                  (call-with-output-file wrapper
                    (lambda (port)
                      (display (string-append "#!/usr/bin/env bash\n"
                                "PATH=\""
                                ncurses-bin
                                ":$PATH\"\n"
