...
;; Web UI on :9090.  Grafana (the grafana VM) reads from it as its default
;; datasource.
;;
;; TWO KINDS OF SCRAPE CONFIG below — know which you need:
;;
;;   1. The "node" job is GENERATED by mapping over %monitored-hosts from
;;      (peteches systems monitored-hosts).  Node-exporter coverage for a
;;      new VM therefore needs NO change to this file — add the host there.
;;
;;   2. Every other job is hand-written, because it needs a service-specific
;;      port, metrics-path, params, or relabelling.  Add one here when a VM
;;      exposes metrics beyond node-exporter's :9100.
;;
;; The hand-written "alloy" job duplicates most of %monitored-hosts at
;; :12345 and must be extended by hand for each new VM — it is the usual
;; thing to forget.
;;
;; The "proxmox" and "snmp" jobs use the exporter-proxy pattern: the target
;; is the *device* being probed, and relabel-configs rewrite __address__ to
;; the exporter that probes it (pve-exporter on 192.168.51.1:9221, the SNMP
;; exporter on 192.168.50.244:9116) while preserving the device as the
;; `instance' label.
