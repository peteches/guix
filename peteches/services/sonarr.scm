;; peteches/services/sonarr.scm — Sonarr TV automation service.

(define-module (peteches services sonarr)
  ...
  #:export (sonarr-configuration sonarr-configuration? sonarr-service-type))

(define-record-type* <sonarr-configuration> sonarr-configuration
                     make-sonarr-configuration
  sonarr-configuration?
  (package
    sonarr-configuration-package
    (default sonarr))
  (port sonarr-configuration-port
        (default 8989))
  (data-dir sonarr-configuration-data-dir
            (default "/var/lib/sonarr"))
  (log-file sonarr-configuration-log-file
            (default "/var/log/sonarr.log")))
...
(define (sonarr-shepherd-service config)
  (let* ((pkg (sonarr-configuration-package config))
    ...
    (list (shepherd-service (provision '(sonarr))
...
(define (sonarr-firewall-rules config)
  (nftables-rules (input (list (string-append "tcp dport "
                                              (number->string (sonarr-configuration-port
                                                               config))
  ...
  (service-type (name 'sonarr)
                ...
                (extensions (list (service-extension account-service-type
                                  ...
                                  (service-extension
                                   ...
                                   sonarr-shepherd-service)
                                  (service-extension firewall-service-type
                                                     sonarr-firewall-rules)
