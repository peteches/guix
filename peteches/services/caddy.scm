;; peteches/services/caddy.scm — Guix service type for Caddy web server.
;;
...
(define-module (peteches services caddy)
  ...
  #:export (caddy-reverse-proxy caddy-reverse-proxy?
...
(define-record-type* <caddy-reverse-proxy> caddy-reverse-proxy
                     make-caddy-reverse-proxy
  caddy-reverse-proxy?
  (domain caddy-reverse-proxy-domain)
  (upstream caddy-reverse-proxy-upstream)
  (tls-backend? caddy-reverse-proxy-tls-backend?
                (default #f))
  (tls-insecure-skip-verify? caddy-reverse-proxy-tls-insecure-skip-verify?
                             (default #f)))
...
(define-record-type* <caddy-configuration> caddy-configuration
  ...
  caddy-configuration?
  (package
    caddy-configuration-package
    (default caddy))
  (email caddy-configuration-email
         (default "pete@peteches.co.uk"))
  (desec-token-file caddy-configuration-desec-token-file
                    (default "/run/secrets/desec-token"))
  (virtual-hosts caddy-configuration-virtual-hosts
                 (default '()))
  (tls-subjects caddy-configuration-tls-subjects
                (default #f))
  (data-path caddy-configuration-data-path
             (default "/var/lib/caddy"))
  (log-file caddy-configuration-log-file
            (default "/var/log/caddy.log"))
  (http-port caddy-configuration-http-port
             (default 80))
  (https-port caddy-configuration-https-port
              (default 443))
  ...
  (dns-target caddy-configuration-dns-target
              (default #f))
  (dns-zone caddy-configuration-dns-zone
            (default #f)))
...
(define (caddy-etc-files config)
  (list `("caddy/caddy.json" ,(plain-file "caddy.json"
                                          (render-caddy-json config)))))
...
(define (caddy-shepherd-service config)
  (let* ((pkg (caddy-configuration-package config))
         ...
         (extra-req (if dns-target
                        '(caddy-ensure-cnames)
                        '())))
    (list (shepherd-service (provision '(caddy))
                            (documentation
                             "Caddy web server with automatic HTTPS via deSEC DNS-01.")
                            ...
                            (start
                             ...
                             #~(make-forkexec-constructor (list "/bin/sh" "-c"
                                                                (string-append
                                                                 "DESEC_TOKEN=$("
                                                                 #$(file-append
                                                                    coreutils
                            ...
                            (actions (list (shepherd-action (name 'reload)
                                                            (documentation
...
(define (caddy-dns-setup-shepherd-service config)
  (let* ((dns-target (caddy-configuration-dns-target config))
         ...
         (desec-token-file (caddy-configuration-desec-token-file config))
         (domains (map caddy-reverse-proxy-domain virtual-hosts)))
    (if (or (not dns-target)
            (not dns-zone))
        '()
        (let* ((curl-bin (file-append curl "/bin/curl"))
               (script (program-file "caddy-ensure-cnames"
                                     #~(begin
                                         ...
                                         (define token
                                           (string-trim-right (call-with-input-file #$desec-token-file
                                                                read-line)))
                                         ...
                                         (define (ensure-cname-records!)
                                           (let* ((existing (fetch-existing-json
                                                             "CNAME"))
                                                  (missing (filter (lambda (d)
                                                                     (not (record-current?
                                                                           existing
                                                                           (fqdn->subname
                                                                            d
                                         ...
                                         (exit 0)))))
          (list (shepherd-service (provision '(caddy-ensure-cnames))
                                  (documentation
                                   "One-shot: create/update deSEC CNAME records for all virtual-host subnames.")
                                  (requirement '(networking sops-secrets))
                                  (one-shot? #t)
                                  (auto-start? #t)
                                  (start #~(make-forkexec-constructor (list #$script)
                                            #:log-file
                                            "/var/log/caddy-ensure-cnames.log"
...
(define (caddy-firewall-rules config)
  (nftables-rules (input (list (string-append "tcp dport "
                                              (number->string (caddy-configuration-http-port
                                                               config))
                                              " accept comment \"caddy-http\"")
                               (string-append "tcp dport "
                                              (number->string (caddy-configuration-https-port
                                                               config))
...
(define-public caddy-service-type
  (service-type (name 'caddy)
                (description
                 "Caddy web server with automatic HTTPS via deSEC DNS-01 challenge.")
                (extensions (list (service-extension activation-service-type
                                  ...
                                  (service-extension
                                   ...
                                   caddy-all-shepherd-services)
                                  (service-extension firewall-service-type
                                                     caddy-firewall-rules)
