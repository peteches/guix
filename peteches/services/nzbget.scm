;; peteches/services/nzbget.scm — NZBGet Usenet downloader service.

(define-module (peteches services nzbget)
  ...
  #:export (nzbget-category nzbget-category?
                            nzbget-news-server
                            nzbget-news-server?
...
(define-record-type* <nzbget-category> nzbget-category make-nzbget-category
  nzbget-category?
  (name nzbget-category-name)
  (dest-dir nzbget-category-dest-dir
            (default #f))
  (unpack? nzbget-category-unpack?
           (default #t))
  (extensions nzbget-category-extensions
              (default '())))

(define-record-type* <nzbget-news-server> nzbget-news-server
                     make-nzbget-news-server
  nzbget-news-server?
  (name nzbget-news-server-name)
  (host nzbget-news-server-host)
  (port nzbget-news-server-port
        (default 563))
  (username nzbget-news-server-username
            (default ""))
  (password-file nzbget-news-server-password-file
                 (default #f))
  (connections nzbget-news-server-connections
               (default 8))
  (encryption? nzbget-news-server-encryption?
               (default #t))
  (retention nzbget-news-server-retention
             (default 0))
  (level nzbget-news-server-level
         (default 0))
  (optional? nzbget-news-server-optional?
             (default #f))
  (active? nzbget-news-server-active?
           (default #t)))

(define-record-type* <nzbget-configuration> nzbget-configuration
                     make-nzbget-configuration
  nzbget-configuration?
  (package
    nzbget-configuration-package
    (default nzbget))
  (port nzbget-configuration-port
        (default 6789))
  (data-dir nzbget-configuration-data-dir
            (default "/var/lib/nzbget"))
  (main-dir nzbget-configuration-main-dir
            (default "/media/downloads/usenet"))
  (nzb-dir nzbget-configuration-nzb-dir
           (default ""))
  (dest-dir nzbget-configuration-dest-dir
            (default #f))
  (temp-dir nzbget-configuration-temp-dir
            (default #f))
  (log-file nzbget-configuration-log-file
            (default "/var/log/nzbget.log"))
  (username nzbget-configuration-username
            (default "nzbget"))
  (password-file nzbget-configuration-password-file
                 (default #f))
  (disk-space nzbget-configuration-disk-space
              (default 250))
  (par-check nzbget-configuration-par-check
             (default "auto"))
  (direct-unpack? nzbget-configuration-direct-unpack?
                  (default #t))
  (append-category-dir? nzbget-configuration-append-category-dir?
                        (default #t))
  (categories nzbget-configuration-categories
              (default '()))
  (news-servers nzbget-configuration-news-servers
                (default '())))
  ...
  (let loop
    ...
    (if (null? cats) result
        (let* ((cat (car cats))
               ...
               (chunk (string-append "Category"
                                     num
                                     ".Name="
                                     name
                                     "\n"
                                     (if dest-dir
                                         (string-append "Category" num
                                                        ".DestDir=" dest-dir
                                                        "\n") "")
                                     "Category"
                                     num
...
Passwords are omitted and injected at startup via --option."
  (let loop
    ...
    (if (null? svrs) result
        (let* ((s (car svrs))
               (num (number->string n))
               (chunk (string-append "Server"
                                     num
                                     ".Active="
                                     (bool->nzbget (nzbget-news-server-active?
                                                    s))
                                     "\n"
                                     "Server"
                                     num
                                     ".Name="
                                     (nzbget-news-server-name s)
                                     "\n"
                                     "Server"
                                     num
                                     ".Host="
                                     (nzbget-news-server-host s)
                                     "\n"
                                     "Server"
                                     num
                                     ".Port="
                                     (number->string (nzbget-news-server-port
                                                      s))
                                     "\n"
                                     "Server"
                                     num
                                     ".Username="
                                     (nzbget-news-server-username s)
                                     "\n"
                                     "Server"
                                     num
                                     ".Connections="
                                     (number->string (nzbget-news-server-connections
                                                      s))
                                     "\n"
                                     "Server"
                                     num
                                     ".Encryption="
                                     (bool->nzbget (nzbget-news-server-encryption?
                                                    s))
                                     "\n"
                                     "Server"
                                     num
                                     ".Retention="
                                     (number->string (nzbget-news-server-retention
                                                      s))
                                     "\n"
                                     "Server"
                                     num
                                     ".Level="
                                     (number->string (nzbget-news-server-level
                                                      s))
                                     "\n"
                                     "Server"
                                     num
                                     ".Optional="
                                     (bool->nzbget (nzbget-news-server-optional?
                                                    s))
                                     "\n"
                                     "Server"
                                     num
...
(define (nzbget-initial-config config)
  ...
  (let* ((main-dir (nzbget-configuration-main-dir config))
    ...
    (string-append "MainDir="
                   ...
                   "\n"
                   "NzbDir="
                   (nzbget-configuration-nzb-dir config)
                   "\n"
                   "QueueDir="
                   (nzbget-configuration-data-dir config)
                   "/queue\n"
                   "LockFile="
                   (nzbget-configuration-data-dir config)
                   "/nzbget.lock\n"
                   "LogFile="
                   (nzbget-configuration-log-file config)
                   "\n"
                   "ControlIP=0.0.0.0\n"
                   "ControlPort="
                   ...
                   "\n"
                   "ControlUsername="
                   (nzbget-configuration-username config)
                   "\n"
                   "SecureControl=no\n"
                   "WriteLog=append\n"
                   "RotateLog=3\n"
                   "AppendCategoryDir="
                   (bool->nzbget (nzbget-configuration-append-category-dir?
                                  config))
                   "\n"
                   "DiskSpace="
                   (number->string (nzbget-configuration-disk-space config))
                   "\n"
                   "ParCheck="
                   (nzbget-configuration-par-check config)
                   "\n"
                   "UnpackPassFile=\n"
                   "DirectUnpack="
                   (bool->nzbget (nzbget-configuration-direct-unpack? config))
                   "\n"
    ...
    #~(begin
        ...
        (let* ((pw (getpwnam "nzbget"))
          ...
          (for-each (lambda (d)
                      (mkdir-p d)
                      (chown d uid gid))
                    (append (list #$data-dir
                                  (string-append #$data-dir "/queue")
                                  #$main-dir
                                  #$dest-dir
                                  #$temp-dir)
                            (if (string-null? #$nzb-dir)
                                '()
                                (list #$nzb-dir))
                            '#$cat-dirs))
...
(define (nzbget-shepherd-service config)
  (let* ((pkg (nzbget-configuration-package config))
         ...
         (news-servers (nzbget-configuration-news-servers config))
         (server-pw-pairs (let loop
                            ((svrs news-servers)
                             (n 1)
                             (acc '()))
                            (if (null? svrs)
                                (reverse acc)
                                (let ((pf (nzbget-news-server-password-file (car
                                                                             svrs))))
                                  (loop (cdr svrs)
                                        (+ n 1)
                                        (if pf
                                            (cons (cons (number->string n) pf)
                                                  acc) acc))))))
         (requirements (if (or password-file
                               (not (null? server-pw-pairs)))
                           '(networking file-systems sops-secrets)
                           '(networking file-systems))))
    (list (shepherd-service (provision '(nzbget))
                                       ...
                                       (let* ((pw-file #$password-file)
                                              (pw-opt (if pw-file
                                                          (list "--option"
                                                                (string-append
                                                                 "ControlPassword="
                                                                 (string-trim-right (call-with-input-file pw-file
                                         ...
                                         (for-each (lambda (pair)
                                                     (let* ((key (string-append
                                                                  ...
                                                                  (car pair)
                                                                  ".Password="))
                                                            (pw (string-trim-right (call-with-input-file 
                                                                                                         (cdr
                                                                                                          pair)
  ...
  (service-type (name 'nzbget)
                ...
                (extensions (list (service-extension account-service-type
                                  ...
                                  (service-extension
                                   ...
                                   nzbget-shepherd-service)
                                  (service-extension firewall-service-type
                                                     nzbget-firewall-rules)
