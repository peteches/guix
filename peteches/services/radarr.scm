;; peteches/services/radarr.scm — Radarr movie automation service.

(define-module (peteches services radarr)
  ...
  #:export (radarr-configuration radarr-configuration? radarr-service-type))

(define-record-type* <radarr-configuration> radarr-configuration
                     make-radarr-configuration
  radarr-configuration?
  (package
    radarr-configuration-package
    (default radarr))
  (port radarr-configuration-port
        (default 7878))
  (data-dir radarr-configuration-data-dir
            (default "/var/lib/radarr"))
  (log-file radarr-configuration-log-file
            (default "/var/log/radarr.log")))
...
(define (radarr-shepherd-service config)
  (let* ((pkg (radarr-configuration-package config))
    ...
    (list (shepherd-service (provision '(radarr))
...
(define (radarr-firewall-rules config)
  (nftables-rules (input (list (string-append "tcp dport "
                                              (number->string (radarr-configuration-port
                                                               config))
  ...
  (service-type (name 'radarr)
                ...
                (extensions (list (service-extension account-service-type
                                  ...
                                  (service-extension
                                   ...
                                   radarr-shepherd-service)
                                  (service-extension firewall-service-type
                                                     radarr-firewall-rules)
