;; peteches/services/jellyfin.scm — Guix service type for Jellyfin.

(define-module (peteches services jellyfin)
  ...
  #:export (jellyfin-configuration jellyfin-configuration?
                                   jellyfin-service-type))

;;; ── Record type ──────────────────────────────────────────────────────────

(define-record-type* <jellyfin-configuration> jellyfin-configuration
                     make-jellyfin-configuration
  jellyfin-configuration?
  (package
    jellyfin-configuration-package
    (default jellyfin))
  (ffmpeg-package jellyfin-configuration-ffmpeg-package
                  (default jellyfin-ffmpeg))
  (http-port jellyfin-configuration-http-port
             (default 8096))
  (data-path jellyfin-configuration-data-path
             (default "/var/lib/jellyfin"))
  (log-path jellyfin-configuration-log-path
            (default "/var/log/jellyfin"))
  (cache-path jellyfin-configuration-cache-path
              (default "/var/cache/jellyfin"))
  ;; Symbol: 'none, 'vaapi, or 'nvenc.
  ;; 'vaapi adds the jellyfin user to "video" and "render" groups.
  ;; 'nvenc adds it to "video" only.
  (hardware-accel jellyfin-configuration-hardware-accel
                  (default 'none))
  ...
  (media-device jellyfin-configuration-media-device
                (default #f))
  (media-mount-point jellyfin-configuration-media-mount-point
                     (default "/media"))
  (media-options jellyfin-configuration-media-options
                 (default "")))
        ...
        (let* ((pw (getpwnam "jellyfin"))
          ...
          (for-each (lambda (d)
                    ...
                    (list #$data-path
                          #$log-path
                          #$cache-path)))
        (system* #$(file-append shepherd "/bin/herd") "restart" "jellyfin"))))
...
(define (jellyfin-shepherd-service config)
  (let* ((pkg (jellyfin-configuration-package config))
         ...
         (log-path (jellyfin-configuration-log-path config))
         (cache-path (jellyfin-configuration-cache-path config))
         (hw (jellyfin-configuration-hardware-accel config))
         (log-file (string-append log-path "/startup.log")))
    (list (shepherd-service (provision '(jellyfin))
                            (documentation
...
(define (jellyfin-firewall-rules config)
  (nftables-rules (input (list (string-append "tcp dport "
                                              (number->string (jellyfin-configuration-http-port
                                                               config))
