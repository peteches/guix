;; peteches/services/prowlarr.scm — Prowlarr indexer manager service.

(define-module (peteches services prowlarr)
  ...
  #:export (prowlarr-configuration prowlarr-configuration?
                                   prowlarr-service-type))

(define-record-type* <prowlarr-configuration> prowlarr-configuration
                     make-prowlarr-configuration
  prowlarr-configuration?
  (package
    prowlarr-configuration-package
    (default prowlarr))
  (port prowlarr-configuration-port
        (default 9696))
  (data-dir prowlarr-configuration-data-dir
            (default "/var/lib/prowlarr"))
  (log-file prowlarr-configuration-log-file
            (default "/var/log/prowlarr.log")))
...
(define (prowlarr-shepherd-service config)
  (let* ((pkg (prowlarr-configuration-package config))
    ...
    (list (shepherd-service (provision '(prowlarr))
...
(define (prowlarr-firewall-rules config)
  (nftables-rules (input (list (string-append "tcp dport "
                                              (number->string (prowlarr-configuration-port
                                                               config))
  ...
  (service-type (name 'prowlarr)
                ...
                (extensions (list (service-extension account-service-type
                                  ...
                                  (service-extension
                                   ...
                                   prowlarr-shepherd-service)
                                  (service-extension firewall-service-type
                                                     prowlarr-firewall-rules)
