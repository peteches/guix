;; peteches/services/transmission.scm — Transmission BitTorrent daemon.

(define-module (peteches services transmission)
  ...
  #:export (transmission-configuration transmission-configuration?
                                       transmission-service-type))

(define-record-type* <transmission-configuration> transmission-configuration
                     make-transmission-configuration
  transmission-configuration?
  (package
    transmission-configuration-package
  ...
  (rpc-authentication-required?
   transmission-configuration-rpc-authentication-required?
   (default #f))
...
(define (transmission-shepherd-service config)
  (let* ((pkg (transmission-configuration-package config))
         ...
         (auth? (transmission-configuration-rpc-authentication-required?
                 config))
    ...
    (list (shepherd-service (provision '(transmission))
                            ...
                            (start #~(lambda args
                                       ...
                                       (let* ((password (and #$auth?
                                                             #$password-file
                                                             (call-with-input-file #$password-file
                                                               (@ (ice-9
                                                                   rdelim)
                                                                  read-line))))
                                              (forkexec (make-forkexec-constructor
...
(define (transmission-firewall-rules config)
  (nftables-rules (input (list (string-append "tcp dport "
                                (number->string (transmission-configuration-rpc-port
                                                 config))
                                " accept comment \"transmission-rpc\"")
                               (string-append "tcp dport "
                                (number->string (transmission-configuration-peer-port
                                                 config))
                                " accept comment \"transmission-peer\"")
                               (string-append "udp dport "
                                (number->string (transmission-configuration-peer-port
                                                 config))
