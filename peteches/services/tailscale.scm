...
;;
(define-module (peteches services tailscale)
  #:use-module (gnu)
  ...
  #:export (tailscale-instance-configuration tailscale-instance-configuration?
  ...
  (match cfg
    (($ <tailscale-instance-configuration>
     ...
     (let* ((defaults (instance-default-paths name))
       ...
       (tailscale-instance-configuration (name name)
                                         (package
                                           package)
    ...
    (match cfg
      (($ <tailscale-instance-configuration>
       ...
       (let* ((service-name (string->symbol (string-append "tailscaled-" name)))
              (run-args (append (list (file-append package "/bin/tailscaled")
                                      (string-append
                                       "--statedir=/var/lib/tailscale/" name))
                                (if socket-file
                                    (list (string-append "--socket="
                                                         socket-file))
                                    '())
                                ...
                                (if port
                                    (list (string-append "--port="
                                                         (number->string port)))
                                    '()) extra-args)))
         (shepherd-service (provision (list service-name))
                           (auto-start? #t)
                           (one-shot? #f)
                           (documentation (string-append "Tailscale daemon ("
                                                         name ")"))
                           (requirement '(user-processes networking))
                           (start #~(make-forkexec-constructor (list #$@run-args)
                                                               #:log-file #$log-file
              ...
              (args (append (list tailscale)
                            ...
                            (list "set"
                                  ...
                                  (string-append "--accept-routes="
                                                 (if accept-routes "true"
                                                     "false"))))))
         (shepherd-service (provision (list svc-name))
                           (documentation (string-append
                                           "Apply persisted Tailscale prefs for instance "
                                           name))
                           (requirement requires)
                           (one-shot? #t)
                           (auto-start? #t)
                           (start #~(make-forkexec-constructor (list #$@args)
                                                               #:log-file #$(string-append
                                                                             "/var/log/tailscale-prefs-"
                                                                             name
                                                                             ".log")
                                                               #:environment-variables
                                                               (list
  ...
  (let ((cfg (instance->resolved cfg)))
    (match cfg
      (($ <tailscale-instance-configuration>
          ...
          taildrop-schedule
          _)
       (if (not taildrop-dir) #f
           (let* ((svc-name (string->symbol (string-append
                                             "tailscale-taildrop-" name)))
                  (tsd-svc (string->symbol (string-append "tailscaled-" name)))
                  (tailscale (file-append package "/bin/tailscale"))
                  (chown-bin (file-append coreutils "/bin/chown"))
                  (lf (string-append "/var/log/tailscale-taildrop-" name
                                     ".log"))
                  (get-script (program-file (string-append
                                             "tailscale-taildrop-get-" name)
                                            #~(begin
                                                (let ((rc (apply system*
                                                                 (append (list #$tailscale)
                                                                         (if #$socket-file
                                                                             (list
                                                                              (string-append
                                                                               "--socket="
             ...
             (shepherd-service (provision (list svc-name))
                               (documentation (string-append
                                               "Poll Taildrop files for Tailscale instance "
                                               name
                                               " into "
                                               taildrop-dir
                                               ". Trigger: herd trigger "
                                               (symbol->string svc-name)))
                               (requirement (list 'user-processes tsd-svc))
                               (modules '((shepherd service timer)))
                               (start #~(make-timer-constructor (cron-string->calendar-event #$taildrop-schedule)
                                                                (command (list #$get-script))))
                               (stop #~(make-timer-destructor))
                               (actions (list (shepherd-action (name 'trigger)
                                                               (documentation
                                                                "Retrieve Taildrop files immediately.")
  ...
  (let ((cfg (instance->resolved cfg)))
    (match cfg
      (($ <tailscale-instance-configuration>
          ...
          _
          auth-key-file)
       (if (not auth-key-file) #f
           (let* ((svc-name (string->symbol (string-append "tailscale-up-"
                                                           name)))
                  (tsd-svc (string->symbol (string-append "tailscaled-" name)))
                  (tailscale (file-append package "/bin/tailscale"))
                  (up-script (program-file (string-append "tailscale-up-" name)
                                           #~(begin
                                               ...
                                               (let loop
                                                 ...
                                                 (cond
                                                   ((zero? n)
                                                    (error
                                                     "tailscale-up: daemon socket not ready after 30s"))
                                                   ((file-exists? sock)
                                                    #t)
                                                   (else (sleep 1)
                                                         (loop (- n 1) sock))))
                                               (let* ((key (string-trim-right (call-with-input-file #$auth-key-file
                                                                                read-line)))
                                                      (rc (apply system*
                                                                 (append (list #$tailscale)
                                                                         (if #$socket-file
             ...
             (shepherd-service (provision (list svc-name))
                               (documentation (string-append
                                               "Enroll Tailscale instance "
                                               name
                                               " using auth-key from file"))
                               (requirement (list 'user-processes
                                                  'networking
                                                  'sops-secrets tsd-svc))
                               (one-shot? #t)
                               (auto-start? #t)
                               (start #~(make-forkexec-constructor (list #$up-script)
                                                                   #:log-file #$
                                                                   (string-append
                                                                    "/var/log/tailscale-up-"
                                                                    name
                                                                    ".log")
                                                                   #:environment-variables
                                                                   (list
...
(define (tailscale-profile-entries instances)
  (let ((resolved (map instance->resolved instances)))
    (delete-duplicates (map tailscale-instance-configuration-package resolved)
                       eq?)))
                       ...
                       #~(begin
                           ...
                           (when #$taildrop-dir
                             ...
                             (when #$taildrop-user
                               (system* #$chown-bin
                                        #$taildrop-user
                                        #$taildrop-dir)))))))) resolved))))

(define (tailscale-shepherd-services instances)
  (append (filter-map tailscale-instance->taildrop-service instances)
