...
(define-module (peteches services restic)
  ...
  #:use-module ((gnu packages backup)
                #:select (restic))
  #:export (restic-vm-backup-configuration restic-vm-backup-configuration?
        ...
        (let ((key-file #$key-file)
          ...
          (unless (zero? (system* #$(file-append restic "/bin/restic")
            ...
            (let ((r (system* #$(file-append restic "/bin/restic")
                              ...
                              "init")))
              (unless (zero? r)
                (display (string-append "restic: WARNING - init failed for "
                          #$repo
                          ".\n"
                          "Ensure "
                          #$key-file
...
(define (restic-vm-shepherd-services cfg)
  (let* ((vm-name (restic-vm-backup-configuration-vm-name cfg))
         ...
         (backup-script (program-file "restic-backup"
                                      #~(begin
                                          (setenv "PATH"
                                          ...
                                          (system* #$(file-append restic
                                                   ...
                                                   #$sftp-o
                                                   #$@paths))))
         (prune-script (program-file "restic-prune"
                                     #~(begin
                                         (setenv "PATH"
                                                 (string-append #$(file-append
                                                                   openssh
                                         ...
                                         (system* #$(file-append restic
                                                  ...
                                                  "--keep-monthly"
                                                  "3")))))
    (list (shepherd-service (provision '(restic-backup))
                            (requirement '(networking))
                            (modules '((shepherd service timer)))
                            (start #~(make-timer-constructor (cron-string->calendar-event #$backup-s)
                            ...
                            (actions (list (shepherd-action (name 'trigger)
                                                            ...
                                                            (procedure #~(lambda (running . _)
                                                                           ...
                                                                           running)))))
                            (documentation
                             "Daily restic backup. Trigger: herd trigger restic-backup"))
          (shepherd-service (provision '(restic-prune))
                            (requirement '(networking))
                            (modules '((shepherd service timer)))
                            (start #~(make-timer-constructor (cron-string->calendar-event #$prune-s)
                                                             (command (list #$prune-script))))
                            (stop #~(make-timer-destructor))
                            (actions (list (shepherd-action (name 'trigger)
                                                            (documentation
                                                             "Run the prune immediately.")
                                                            (procedure #~(lambda (running . _)
                                                                           (system* #$prune-script)
                                                                           running)))))
...
(define restic-vm-backup-service-type
  (service-type (name 'restic-vm-backup)
                (extensions (list (service-extension activation-service-type
                                   restic-vm-backup-activation)
                                  (service-extension
                                   shepherd-root-service-type
                                   restic-vm-shepherd-services)))
                (description
                 "VM backup service using restic over SFTP to a Synology NAS.  Fetches the NAS host key via ssh-keyscan (TOFU) and initializes the restic repository if missing.  Secrets (password and SSH key) are provided via SOPS.  Each scheduled backup also auto-inits if needed.  Trigger manually with `herd trigger restic-backup`.")))
