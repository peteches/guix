;; peteches/services/prometheus.scm — Guix service type for Prometheus.

(define-module (peteches services prometheus)
  ...
  #:export (prometheus-global-config prometheus-global-config?
...
(define-record-type* <prometheus-global-config> prometheus-global-config
                     make-prometheus-global-config
  prometheus-global-config?
  (scrape-interval prometheus-global-config-scrape-interval
                   (default "15s"))
  (evaluation-interval prometheus-global-config-evaluation-interval
                       (default "15s"))
  (scrape-timeout prometheus-global-config-scrape-timeout
                  (default #f))
  (external-labels prometheus-global-config-external-labels
                   (default '())))

(define-record-type* <prometheus-relabel-config> prometheus-relabel-config
                     make-prometheus-relabel-config
  prometheus-relabel-config?
  (source-labels prometheus-relabel-config-source-labels
                 (default '()))
  (separator prometheus-relabel-config-separator
             (default #f))
  (target-label prometheus-relabel-config-target-label
                (default #f))
  (regex prometheus-relabel-config-regex
         (default #f))
  (modulus prometheus-relabel-config-modulus
           (default #f))
  (replacement prometheus-relabel-config-replacement
               (default #f))
  (action prometheus-relabel-config-action
          (default #f)))

(define-record-type* <prometheus-static-config> prometheus-static-config
                     make-prometheus-static-config
  prometheus-static-config?
...
(define-record-type* <prometheus-scrape-config> prometheus-scrape-config
                     make-prometheus-scrape-config
  prometheus-scrape-config?
  (job-name prometheus-scrape-config-job-name)
  (scrape-interval prometheus-scrape-config-scrape-interval
                   (default #f))
  (scrape-timeout prometheus-scrape-config-scrape-timeout
                  (default #f))
  (metrics-path prometheus-scrape-config-metrics-path
                (default #f))
  (scheme prometheus-scrape-config-scheme
          (default #f))
  (params prometheus-scrape-config-params
          (default '()))
  (static-configs prometheus-scrape-config-static-configs
                  (default '()))
  (relabel-configs prometheus-scrape-config-relabel-configs
                   (default '()))
  (metric-relabel-configs prometheus-scrape-config-metric-relabel-configs
                          (default '())))

(define-record-type* <prometheus-configuration> prometheus-configuration
                     make-prometheus-configuration
  prometheus-configuration?
  (package
    prometheus-configuration-package
    (default prometheus))
  (global prometheus-configuration-global
          (default (prometheus-global-config)))
  (rule-files prometheus-configuration-rule-files
              (default '()))
  (scrape-configs prometheus-configuration-scrape-configs
                  (default '()))
  (web-listen-address prometheus-configuration-web-listen-address
                      (default "0.0.0.0:9090"))
  (storage-path prometheus-configuration-storage-path
                (default "/var/lib/prometheus"))
  (log-file prometheus-configuration-log-file
            (default "/var/log/prometheus.log"))
  (extra-args prometheus-configuration-extra-args
              (default '())))
...
(define (render-global g)
  (string-append "global:\n"
                 "  scrape_interval: "
                 (prometheus-global-config-scrape-interval g)
                 "\n"
                 "  evaluation_interval: "
                 (prometheus-global-config-evaluation-interval g)
                 "\n"
                 (let ((t (prometheus-global-config-scrape-timeout g)))
                   (if t
                       (string-append "  scrape_timeout: " t "\n") ""))
                 (let ((el (prometheus-global-config-external-labels g)))
                   (if (null? el) ""
                       (string-append "  external_labels:\n"
                                      (apply string-append
                                             (map (lambda (kv)
                                                    (string-append "    "
                                                                   (car kv)
  ...
  (let ((src (prometheus-relabel-config-source-labels rc))
    ...
    (let* ((fields (filter (lambda (x)
                           ...
                           (list (and (not (null? src))
                                 ...
                                 (and sep
                                      (lambda (m)
                                        (string-append m "separator: "
                                                       (yaml-quote sep) "\n")))
                                 (and tgt
                                      (lambda (m)
                                        (string-append m "target_label: "
                                                       (yaml-quote tgt) "\n")))
                                 (and rex
                                      (lambda (m)
                                        (string-append m "regex: "
                                                       (yaml-quote rex) "\n")))
                                 (and mod
                                      (lambda (m)
                                        (string-append m "modulus: "
                                                       (number->string mod)
                                                       "\n")))
                                 (and rep
                                      (lambda (m)
                                        (string-append m "replacement: "
                                                       (yaml-quote rep) "\n")))
                                 (and act
                                      (lambda (m)
                                        (string-append m "action: " act "\n"))))))
           (first-fn (and (not (null? fields))
                          (car fields)))
           (rest-fns (if (null? fields)
                         '()
                         (cdr fields))))
      (if (not first-fn) ""
          (string-append (first-fn "      - ")
                         (apply string-append
                                (map (lambda (fn)
                                       (fn "        ")) rest-fns)))))))
...
(define (prometheus-activation config)
  (let ((storage-path (prometheus-configuration-storage-path config)))
    #~(begin
        ...
        (system* #$(file-append shepherd "/bin/herd") "restart" "prometheus"))))

(define (prometheus-etc-files config)
  (list `("prometheus/prometheus.yml" ,(plain-file "prometheus.yml"
                                                   (render-prometheus-yaml
                                                    config)))))
...
(define (prometheus-shepherd-service config)
  (let* ((pkg (prometheus-configuration-package config))
    ...
    (list (shepherd-service (provision '(prometheus))
...
(define (prometheus-firewall-rules config)
  (let ((port (web-address->port (prometheus-configuration-web-listen-address
                                  config))))
