;; peteches/services/firewall.scm
;;
...
(define-module (peteches services firewall)
  ...
  #:export (nftables-rules nftables-rules? rules-merge firewall-service-type))
...
(define-record-type* <nftables-rules> nftables-rules make-nftables-rules
  nftables-rules?
  (input nftables-rules-input
         (default '()))
  (forward nftables-rules-forward
           (default '()))
  (nat-postrouting nftables-rules-nat-postrouting
                   (default '()))
  (nat-output nftables-rules-nat-output
              (default '()))
  ;; Raw text appended verbatim at the end (e.g. extra tables, sets, maps).
  (raw nftables-rules-raw
       (default '())))

;; Merge two fragments.
(define-public (rules-merge a b)
  (nftables-rules (input (append (nftables-rules-input a)
                                 (nftables-rules-input b)))
                  (forward (append (nftables-rules-forward a)
                                   (nftables-rules-forward b)))
                  (nat-postrouting (append (nftables-rules-nat-postrouting a)
                                           (nftables-rules-nat-postrouting b)))
                  (nat-output (append (nftables-rules-nat-output a)
                                      (nftables-rules-nat-output b)))
                  (raw (append (nftables-rules-raw a)
                               (nftables-rules-raw b)))))
...
(define (firewall-activation rules)
  #~(when (file-exists? "/run/shepherd/socket")
      (system* #$(file-append shepherd "/bin/herd") "reload"
               "peteches-firewall")))

(define (firewall-shepherd-service rules)
  (let ((ruleset (firewall-ruleset-file rules)))
    (list (shepherd-service (provision '(peteches-firewall))
                            (documentation
                             "Apply peteches modular nftables firewall ruleset.")
                            ;; Run late enough that interfaces exist and routing is sane.
                            (requirement '(user-processes networking))
                            (auto-start? #t)
                            (one-shot? #t)
                            (start #~(lambda _
                                       (let ((nft #$(file-append nftables
                                                                 "/sbin/nft")))
                                         (format #t
                                          "peteches-firewall: applying ~a\n"
                                          #$ruleset)
                                         (let ((rc (system* nft "-f"
                                                            #$ruleset)))
                                           (unless (zero? rc)
                                             (error
                                              "peteches-firewall: nft failed"
                                              rc))) #t)))
                                                                           ...
                                                                           (let 
                                                                             ...
                                                                             (let 
                                                                               ...
                                                                               (unless 
                                                                                 ...
                                                                                 (error
                                                                                  "peteches-firewall: nft reload failed"
                                                                                  rc)))
                                                                             #t))))))
                            (stop #~(lambda _
                                      #t))))))
...
(define-public firewall-service-type
  (service-type (name 'peteches-firewall)
                ...
                (description
                 "Modular, authoritative nftables firewall: merges fragments, writes /etc/nftables.conf, and applies it via Shepherd.")))
