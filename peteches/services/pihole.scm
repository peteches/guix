;; peteches/services/pihole.scm — Guix service type for Pi-hole.
;;
...
(define-module (peteches services pihole)
  ...
  #:export (pihole-unbound-configuration pihole-unbound-configuration?
                                         pihole-custom-host
                                         pihole-custom-host?
...
(define-record-type* <pihole-custom-host> pihole-custom-host
                     make-pihole-custom-host
  pihole-custom-host?
...
(define-record-type* <pihole-unbound-configuration>
                     ...
                     make-pihole-unbound-configuration
  pihole-unbound-configuration?
  (listen-port pihole-unbound-configuration-listen-port
               (default 5335))
...
(define (render-pihole-toml config)
  ...
  (let* ((with-ub? (pihole-configuration-with-unbound? config))
         ...
         (upstreams (if with-ub?
                        (list (string-append (pihole-unbound-configuration-listen-address
                                              ub) "#"
                                             (number->string (pihole-unbound-configuration-listen-port
                                                              ub))))
    ...
    (string-append "[dns]\n"
                   "  upstreams = "
                   (toml-string-array upstreams)
                   "\n"
                   "  interface = "
                   (toml-string (pihole-configuration-interface config))
                   "\n"
                   "  listeningMode = "
                   (toml-string (pihole-configuration-listening-mode config))
                   "\n"
                   "  queryLogging = "
                   (toml-bool (pihole-configuration-query-logging? config))
                   "\n"
                   "  blocking.active = true\n"
                   "  blocking.mode = "
                   (toml-string (pihole-configuration-blocking-mode config))
                   "\n"
                   "  hosts = "
                   (toml-string-array hosts)
                   "\n"
                   "\n"
                   "[files]\n"
                   "  database = "
                   (toml-string (string-append data-dir "/pihole-FTL.db"))
                   "\n"
                   "  gravity = "
                   (toml-string (string-append data-dir "/gravity.db"))
                   "\n"
                   "  gravity_tmp = \"/tmp\"\n"
                   "  macvendor = "
                   (toml-string (string-append data-dir "/macvendor.db"))
                   "\n"
                   "  log.ftl = "
                   (toml-string (string-append log-dir "/FTL.log"))
                   "\n"
                   "  log.dnsmasq = "
                   (toml-string (string-append log-dir "/pihole.log"))
                   "\n"
                   "  log.webserver = "
                   (toml-string (string-append log-dir "/webserver.log"))
                   "\n"
                   "\n"
                   "[webserver]\n"
                   "  port = "
                   (toml-string (if web-port
                                    (number->string web-port) ""))
                   "\n"
                   (if (string=? pw-hash "") ""
                       (string-append "  api.password = "
                                      (toml-string pw-hash) "\n"))
                   "\n"
                   "[ntp]\n"
                   "  sync.active = false\n"
...
(define (render-unbound-conf config)
  ...
  (let* ((ub (pihole-configuration-unbound config))
    ...
    (string-append "server:\n"
                   ...
                   xtra
                   "\n"
                   (string-join (map (lambda (zone)
                                       (string-append "forward-zone:\n"
                                                      "    name: \""
                                                      (car zone)
                                                      "\"\n"
                                                      "    forward-addr: "
...
(define (render-custom-hosts hosts)
  ...
  (if (null? hosts) ""
      (string-append (string-join (map (lambda (h)
                                         (string-append (pihole-custom-host-address
                                                         h) " "
                                                        (pihole-custom-host-hostname
                                                         h))) hosts) "\n")
                     "\n")))
...
(define (render-adlist-sql adlists)
  ...
  (if (null? adlists) ""
      (string-append "BEGIN TRANSACTION;\n"
                     (string-join (map (lambda (url)
                                         (string-append
                                          "INSERT OR IGNORE INTO adlist (address,enabled,comment)"
                                          " VALUES ('"
                                          url
                                          "',1,'Added by Guix configuration');\n"
                                          "INSERT OR IGNORE INTO adlist_by_group (adlist_id, group_id)"
                                          " SELECT id, 0 FROM adlist WHERE address='"
                                          url
                                          "';")) adlists) "\n") "\nCOMMIT;\n")))
...
(define (pihole-activation config)
  (let* ((pkg (pihole-configuration-package config))
         ...
         (toml-file (plain-file "pihole.toml"
                                (render-pihole-toml config)))
         (custom-list (plain-file "custom.list"
                                  (render-custom-hosts (pihole-configuration-custom-hosts
                                                        config))))
         (sql-file (plain-file "gravity.db.sql" %gravity-db-sql))
         (adlist-sql (plain-file "adlist-seed.sql"
                                 (render-adlist-sql adlists))))
    #~(begin
        ...
        (symlink #$(file-append pihole-web "/share/pihole-web")
                 "/var/www/html")
        ...
        (let ((gravity-db (string-append #$data-dir "/gravity.db")))
          (unless (file-exists? gravity-db)
            (invoke #$(file-append pkg "/bin/pihole-FTL") "sqlite3" gravity-db
                    (string-append ".read "
                                   #$sql-file) ".quit")
            (let* ((pw (getpwnam "pihole"))
                   (uid (passwd:uid pw))
        ...
        (invoke #$(file-append pkg "/bin/pihole-FTL") "sqlite3"
                (string-append #$data-dir "/gravity.db")
                (string-append ".read "
                               #$adlist-sql) ".quit")
        ...
        (system (string-append "su -s /bin/sh pihole -c '"
...
(define (pihole-etc-files config)
  ...
  (if (pihole-configuration-with-unbound? config)
      (list `("unbound/unbound.conf" ,(plain-file "unbound.conf"
                                                  (render-unbound-conf config))))
      '()))

(define (make-unbound-shepherd-service config)
  (shepherd-service (provision '(unbound))
                    (documentation
...
(define (make-pihole-exporter-shepherd-service config)
  (let* ((pkg (pihole-configuration-exporter-package config))
    ...
    (shepherd-service (provision '(pihole-exporter))
                      (documentation
                       "Prometheus exporter for Pi-hole statistics.")
                      (requirement '(pihole networking))
                      (start #~(make-forkexec-constructor (append (list #$(file-append
                                                                           pkg
                                                                           "/bin/pihole-exporter")
                                                                   "-port"
                                                                   #$(number->string
...
(define (pihole-shepherd-service config)
  (let* ((pkg (pihole-configuration-package config))
         ...
         (ftl-svc (shepherd-service (provision '(pihole))
                                    (documentation
                                     "Pi-hole FTL DNS ad-blocking daemon.")
                                    ...
                                    (start
                                     ...
                                     #~(let ((make-it (make-forkexec-constructor
                                                       (append (list #$(file-append
                                                                        pkg
                                                                        "/bin/pihole-FTL")
                                                                "no-daemon")
                                                               '#$(pihole-configuration-extra-args
                                                                   config))
                                                       #:log-file #$(string-append
                                                                     log-dir
                                                                     "/FTL.log"))))
    ...
    (let ((services (if with-ub?
      ...
      (if (pihole-configuration-with-exporter? config)
          (append services
                  (list (make-pihole-exporter-shepherd-service config)))
          services))))

(define (pihole-firewall-rules config)
  (let ((web-port (pihole-configuration-web-port config)))
    (nftables-rules (input (append (list
                                    "udp dport 53 accept comment \"pihole-dns\""
                                   ...
                                   (if web-port
                                       ...
                                       '())
                                   (if (pihole-configuration-with-exporter?
                                        config)
                                       (list (string-append "tcp dport "
                                              (number->string (pihole-configuration-exporter-port
                                                               config))
                                              " accept comment \"pihole-exporter\""))
                                       '()))))))

(define (pihole-profile config)
  (append (list (pihole-configuration-package config) pihole-scripts curl
...
web interface.")
                (extensions (list (service-extension account-service-type
                                  ...
                                  (service-extension
                                   ...
                                   pihole-shepherd-service)
                                  (service-extension firewall-service-type
                                                     pihole-firewall-rules)
