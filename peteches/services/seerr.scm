;; peteches/services/seerr.scm — Seerr media request manager service.

(define-module (peteches services seerr)
  ...
  #:export (seerr-configuration seerr-configuration? seerr-service-type))

(define-record-type* <seerr-configuration> seerr-configuration
                     make-seerr-configuration
  seerr-configuration?
  (package
    seerr-configuration-package
    (default seerr))
  (port seerr-configuration-port
        (default 5055))
  (data-dir seerr-configuration-data-dir
            (default "/var/lib/seerr"))
  (log-file seerr-configuration-log-file
            (default "/var/log/seerr.log")))
...
(define (seerr-shepherd-service config)
  (let* ((pkg (seerr-configuration-package config))
    ...
    (list (shepherd-service (provision '(seerr))
...
(define (seerr-firewall-rules config)
  (nftables-rules (input (list (string-append "tcp dport "
                                              (number->string (seerr-configuration-port
                                                               config))
  ...
  (service-type (name 'seerr)
                ...
                (extensions (list (service-extension account-service-type
                                  ...
                                  (service-extension
                                   ...
                                   seerr-shepherd-service)
                                  (service-extension firewall-service-type
                                                     seerr-firewall-rules)
