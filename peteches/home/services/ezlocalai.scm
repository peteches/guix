...
(define-record-type* <ezlocalai-direct-home-configuration>
                     ezlocalai-direct-home-configuration
                     make-ezlocalai-direct-home-configuration
  ezlocalai-direct-home-configuration?
  ;; state & secrets (under $HOME):
  (state-relpath ez-state-rel
                 (default "/.local/share/ezlocalai"))
  (venv-relpath ez-venv-rel
                (default "/.local/share/ezlocalai/venv"))
  (secrets-relpath ez-secrets-rel
                   (default "/.local/share/ezlocalai/secrets.env"))
  ;; ports
  (http-port ez-http-port
             (default 8091))
  (ui-port ez-ui-port
           (default 8502))
  ;; auto-update: re-run pip on start
  (auto-update? ez-auto-update?
                (default #t))
  ;; extra requirements written to ~/.config/ezlocalai/requirements.txt
  ;; Provide ORT by default to fix Embedding import.
  (extra-requirements ez-extra-reqs
                      (default '("optimum[onnxruntime]==1.18.1"
                                 "onnxruntime>=1.16,<2"
                                 "transformers==4.39.3"
                                 "diffusers==0.29.2"
  ...
  (let* ((pairs (ez-env-over cfg))
         (->line (lambda (p)
                   (unless (and (pair? p)
                                ...
                                (string? (cdr p)))
                     (error
                      "env-overrides must be list of (STRING . STRING) pairs"
                      p))
                   (string-append (car p) "="
                                  (cdr p)))))
...
(define (svc cfg)
  (shepherd-service (documentation
                     "ezlocalai: Uvicorn API + Streamlit UI from a local venv.")
                    (provision '(ezlocalai))
...
(define ezlocalai-direct-home-service-type
  (service-type (name 'ezlocalai-direct-home)
                (extensions (list
                             ...
                             (service-extension
                              ...
                              (lambda (cfg)
                                `(("ezlocalai/ezlocalai.env" ,(ezlocalai-env-file
                                                               cfg))
                                  ("ezlocalai/requirements.txt" ,(extra-reqs-file
                                                                  cfg))
                                  ("ezlocalai/run.sh" ,(ezlocalai-run-script
                                                        cfg)))))
                             ;; Shepherd service
                ...
                (description
                 "Run ezlocalai directly via Python/uvicorn + Streamlit; simple and reproducible.")))
