...
(define-module (peteches home services git)
  ...
  #:export (home-git-configuration git-section git-section-name
                                   git-section-config))
...
Examples: \"Global/Linux\", \"CommonLisp\"."
  (let ((sanitized (string-append "gitignore-"
                                  (string-replace-substring name "/" "-"))))
    (computed-file (string-append "gitignore-" sanitized)
                   ;; Output should be a *file*, not a directory.
                   #~(copy-file (string-append #$%gitignore-source "/"
                                               #$name ".gitignore")
                                #$output))))
...
(define (serialize-git-section section)
  (string-append "["
                 (git-section-name section) "]\n"
                 (serialize-list-of-key-values "c"
                                               (git-section-config section))
                 "\n"))
...
; for 'guile-3.0' below (top of file once)
...
(define (hook-file* label files)
  (if (null? files)
      ...
      (program-file label
                    #~(exit 0)
                    #:guile guile-3.0)
      (let ((payload (mixed-text-file (string-append label ".payload")
...
(define (home-git-files-service-type config)
  (list
   ;; Hooks
   `("git/hooks/applypatch-msg" ,(hook-file* "applypatch-msg"
                                             (home-git-configuration-applypatch-msg-hook
                                              config)))
   `("git/hooks/commit-msg" ,(hook-file* "commit-msg"
                                         (home-git-configuration-commit-msg-hook
                                          config)))
   `("git/hooks/fsmonitor-watchman" ,(hook-file* "fsmonitor-watchman"
                                                 (home-git-configuration-fsmonitor-watchman-hook
                                                  config)))
   `("git/hooks/post-merge" ,(hook-file* "post-merge"
                                         (home-git-configuration-post-merge-hook
                                          config)))
   `("git/hooks/post-update" ,(hook-file* "post-update"
                                          (home-git-configuration-post-update-hook
                                           config)))
   `("git/hooks/pre-merge-commit" ,(hook-file* "pre-merge-commit"
                                               (home-git-configuration-pre-merge-commit-hook
                                                config)))
   `("git/hooks/pre-rebase" ,(hook-file* "pre-rebase"
                                         (home-git-configuration-pre-rebase-hook
                                          config)))
   `("git/hooks/pre-receive" ,(hook-file* "pre-receive"
                                          (home-git-configuration-pre-receive-hook
                                           config)))
   `("git/hooks/pre-applypatch" ,(hook-file* "pre-applypatch"
                                             (home-git-configuration-pre-applypatch-hook
                                              config)))
   `("git/hooks/pre-commit" ,(hook-file* "pre-commit"
                                         (home-git-configuration-pre-commit-hook
                                          config)))
   `("git/hooks/pre-push" ,(hook-file* "pre-push"
                                       (home-git-configuration-pre-push-hook
                                        config)))
   `("git/hooks/prepare-commit-msg" ,(hook-file* "prepare-commit-msg"
                                                 (home-git-configuration-prepare-commit-msg-hook
                                                  config)))
   `("git/hooks/push-to-checkout" ,(hook-file* "push-to-checkout"
                                               (home-git-configuration-push-to-checkout-hook
                                                config)))
   `("git/hooks/sendemail-validate" ,(hook-file* "sendemail-validate"
                                                 (home-git-configuration-sendemail-validate-hook
                                                  config)))
   `("git/hooks/update" ,(hook-file* "update"
                                     (home-git-configuration-update-hook
                                      config)))

   ;; Global ignore file
   `("git/ignore" ,(mixed-text-file "global-ignore"
                                    (serialize-text-config "ignore"
                                                           (home-git-configuration-global-ignore
                                                            config))))

   ;; Global config (ensures hooksPath & excludesFile)
   `("git/config" ,(mixed-text-file "config"
                                    (serialize-list-of-git-sections "config"
                                                                    (home-git-configuration-config
                                                                     config))))))
