...
(define-module (peteches home services firefox)
  ...
  #:export (firefox-configuration firefox-profile-decl
                                  firefox-profile
...
(define-record-type* <firefox-profile-decl> firefox-profile-decl
                     make-firefox-profile-decl
  firefox-profile-decl?
  (name firefox-profile-decl-name)
  (id firefox-profile-decl-id)
  (path firefox-profile-decl-path
        (default #f))
  (prefs firefox-profile-decl-prefs
         (default '()))
  (extensions firefox-profile-decl-extensions
              (default '())))

(define* (firefox-profile name
                          id
                          #:key (path #f)
                          (prefs '())
                          (extensions '()))
  (firefox-profile-decl (name name)
                        (id id)
                        (path path)
                        (prefs prefs)
                        (extensions extensions)))

(define-record-type* <firefox-configuration> firefox-configuration
                     make-firefox-configuration
  firefox-configuration?
  (firefox-package firefox-conf-firefox-package
                   (default firefox))
  (wofi-package firefox-conf-wofi-package
                (default wofi))
  (xdg-utils-package firefox-conf-xdg-package
                     (default xdg-utils))
  (wofi-cmd firefox-conf-wofi-cmd
            (default "wofi --dmenu --prompt 'Select Firefox Profile:'"))
  (profiles firefox-conf-profiles)
  (default-profile firefox-conf-default-profile
                   (default #f))
  (global-extensions firefox-conf-global-extensions
                     (default '()))
  (global-prefs firefox-conf-global-prefs
                (default '())))
...
(define (profile-path p)
  (let ((pth (firefox-profile-decl-path p)))
    (if (and pth
             (string? pth)) pth
        (string-append "profiles/"
                       (firefox-profile-decl-id p)))))
...
"
                       (apply string-append
                              (reverse acc)))
        (let* ((p (car ps))
               (s (string-append "[Profile"
                                 (number->string i)
                                 "]\n"
                                 "Name="
                                 (firefox-profile-decl-name p)
                                 "\n"
                                 "IsRelative=1\n"
                                 "Path="
                                 (profile-path p)
                                 "\n"
                                 (if (and default-name
                                          (string=? (firefox-profile-decl-name
                                                     p) default-name))
                                     "Default=1\n\n" "\n"))))
          (loop (cdr ps)
                (+ i 1)
...
(define (merge-prefs global local)
  (let loop
    ((xs (append global local))
     (acc '()))
    (if (null? xs)
        (reverse acc)
        (let* ((kv (car xs))
               (key (car kv))
               (acc2 (filter (lambda (p)
                               (not (equal? (car p) key))) acc)))
          (loop (cdr xs)
                (cons kv acc2))))))
...
(define* (firefox-with-policies #:key (base firefox)
                                (extensions '())
                                (tag "p"))
  (let* ((ensure (lambda (p)
                   (cons (car p)
                         (ensure-xpi-file (cdr p)))))
         (ext-pairs (map ensure extensions))
         (xpi-inputs (map (lambda (p)
                            (list (string-append "xpi-"
                                                 (car p))
                                  (cdr p))) ext-pairs))
...
"
      ...
      (description
       "Copies Firefox into a new store item and overlays distribution/policies.json and distribution/extensions/*.xpi so Firefox force-installs the selected extensions.")
...
(define (firefox-launcher-package cfg wrappers)
  ...
  (let* ((launcher-src (local-file "./firefox-profile-launcher.sh"))
         ...
         (labels+inputs (map (lambda (pw)
                               (let* ((p (car pw))
                                      (pkg (cdr pw))
                                      (name (firefox-profile-decl-name p))
                                      (lab (string-append "ff-"
                                                          (firefox-profile-decl-id
                                                           p))))
                                 (list name lab pkg))) wrappers)))
    (package
                                         ...
                                         #~(begin
                                             ...
                                             (let* ((out (assoc-ref %outputs
                                                    ...
                                                    (share (string-append out
                                                            "/share"))
                                                    (dst (string-append bin
                                                          "/firefox-profile-launcher"))
                                                    (src (assoc-ref
                                                          %build-inputs
                                                          "launcher-src"))
                                                    (mapf (string-append share
                                                           "/firefox-wrappers.txt"))
                                                    (name+labels '#$(map (lambda 
                                                                                 (nlp)
                                                                           (list
                                                                            (car
                                                                             nlp)
      ...
      (description
       "Copies an external shell launcher and a generated map of profile names to wrapped Firefox binaries; the launcher uses wofi to select and exec the right binary with -P <Name>.")
      ...
      (concatenate (map (lambda (p)
                          (let* ((pid (firefox-profile-decl-id p))
                                 ...
                                 (prefs (merge-prefs global-prefs
                                                     (firefox-profile-decl-prefs
                                                      p)))
                                 (userjs (plain-file (string-append pid
                                                      "-user.js")
                                                     (serialize-user-js prefs))))
                            (list (list (string-append dir "user.js") userjs))))
                        profiles)))
    (append (list (list ".mozilla/firefox/profiles.ini"
                        (plain-file "profiles.ini"
                                    (serialize-profiles-ini profiles
                                                            default-name))))
            per-profile-entries)))
...
(define (firefox-desktop-service config)
  (let* ((desktop-id "firefox-profile-launcher.desktop")
         (desktop-file (plain-file desktop-id
...
(define (firefox-mime-service config)
  (home-xdg-mime-applications-configuration (default '(("x-scheme-handler/http" . "firefox-profile-launcher.desktop")
...
(define-public firefox-service-type
  (service-type (name 'firefox-config)
                (extensions (list (service-extension home-profile-service-type
                                   (lambda (cfg)
                                     (let* ((base (firefox-conf-firefox-package
                                                   cfg))
                                            (globals (firefox-conf-global-extensions
                                                      cfg))
                                            (profiles (firefox-conf-profiles
                                                       cfg))
                                            (wrappers (map (lambda (p)
                                                             (let* ((per (firefox-profile-decl-extensions
                                                                          p))
                                                                    (all (append
                                                                          globals
                                                                          per))
                                                               ...
                                                               (cons p pkg)))
                                                           profiles))
                                            (launcher (firefox-launcher-package
                                                       cfg wrappers)))
                                       (append (list (firefox-conf-wofi-package
                                                      cfg)
                                                     (firefox-conf-xdg-package
                                                      cfg) launcher)
                                               (map cdr wrappers)))))
