...
(define-module (peteches home services koboldcpp)
  ...
  #:export (koboldcpp-configuration koboldcpp-service-type))
...
(define-record-type* <koboldcpp-configuration> koboldcpp-configuration
  ...
  (package
    koboldcpp-configuration-package
    (default koboldcpp-bin))
  (model-path koboldcpp-configuration-model-path
              (default (string-append (or (getenv "HOME") "")
                                      "/.local/share/models")))
  ...
  (work-dir koboldcpp-configuration-work-dir
            (default (string-append (or (getenv "HOME") "")
...
(define (koboldcpp-shepherd-service cfg)
  (let* ((pkg (koboldcpp-configuration-package cfg))
         ...
         (service-name (string->symbol (koboldcpp-configuration-service-name
                                        cfg)))
         ...
         (extra-args (append (koboldcpp-configuration-extra-args cfg)
    ...
    (list (shepherd-service (documentation
                             "KoboldCpp text-generation server (user session).")
...
(define koboldcpp-service-type
  (service-type (name 'koboldcpp)
                (extensions (list (service-extension
                                   home-activation-service-type create-dir)
                                  (service-extension
