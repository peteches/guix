(define-module (peteches home services mpv)
  ...
  #:export (mpv-config mpv-profile mpv-profile? home-mpv-service-type))
...
Examples:
  ...
  (cond
    ...
    ((string? key)
     key)
    (else (error "unsupported mpv option name" key))))
...
Examples:
  ...
  (cond
    ...
    ((symbol? value)
     (symbol->string value))
    (else (error "unsupported mpv configuration value" value))))
...
Both symbols and strings are accepted as option names."
  #~(string-append #$@(map (match-lambda
                             ((key . value) (serialize-option (mpv-option-name
                                                               key) value)))
                           options)))
...
(define-record-type* <mpv-profile> mpv-profile make-mpv-profile
  mpv-profile?
...
every default."
  #~(string-append
     ...
     #$(serialize-option-alist (mpv-profile-extra-options profile))))
...
Impact:
  ...
  (save-position-on-quit? (boolean #t)
...
Impact:
  ...
  (screenshot-dir (string "~/Pictures/mpv")
...
Impact:
  ...
  (screenshot-template (string "%F-%wH.%wM.%wS.%wT-#%#00n")
...
Impact:
  ...
  (demuxer-max-bytes (string "1024MiB")
...
Impact:
  ...
  (demuxer-max-back-bytes (string "512MiB")
...
Impact:
  ...
  (demuxer-readahead-secs (integer 600)
...
Impact:
  ...
  (demuxer-hysteresis-secs (integer 10)
...
(define (home-mpv-profile-files config)
  ...
  (map (lambda (profile)
         `(,(profile-name->file-name (mpv-profile-name profile)) ,(mixed-text-file
                                                                   (string-append
                                                                    "mpv-profile-"
                                                                    (mpv-profile-name
                                                                     profile)
                                                                    ".conf")
                                                                   (serialize-mpv-profile
                                                                    profile))))
...
(define (home-mpv-xdg-files config)
  ...
  (append (list `("mpv/mpv.conf" ,(mixed-text-file "mpv.conf"
                                                   (serialize-configuration
                                                    config mpv-config-fields))))
          (home-mpv-profile-files config)))

(define-public home-mpv-service-type
  (service-type (name 'mpv-config)
                (description
