  ...
  (list
   ...
   (simple-service 'matugen-material-style
		   ...
		   `(("alacritty/alacritty.toml"
		     ...
		     ("matugen/templates/herdr-config.toml"
		      ,(local-file (source-path "configs/matugen/templates/herdr-config.toml")))))
   ...
   (simple-service 'matugen-dms-custom-templates
		   ...
		   #~(begin
		      ...
		      (herdr-dir (string-append home "/.config/herdr"))
		      ...
		      (herdr-config (string-append herdr-dir "/config.toml"))
			 ...
			 (call-with-output-file config-file
			   (lambda (port)
			     ...
			     (display "[templates.herdr]\n" port)
			     (display (string-append "input_path = '" home "/.config/matugen/templates/herdr-config.toml'\n") port)
			     (display (string-append "output_path = '" herdr-config "'\n") port)
			     (display "post_hook = \"herdr server reload-config >/dev/null 2>&1 || true\"\n\n" port)))
			 ...
			 (unless (file-exists? herdr-config)
			   (call-with-output-file herdr-config
			     (lambda (port)
			       (display "# Fallback herdr config — replaced by matugen on first wallpaper change.\n" port)
			       (display "onboarding = false\n\n" port)
			       (display "[ui]\n" port)
			       (display "show_agent_labels_on_pane_borders = true\n\n" port)
			       (display "[ui.toast]\n" port)
			       (display "delivery = \"system\"\n\n" port)
			       (display "[theme]\n" port)
			       (display "name = \"gruvbox\"\n" port)
			       (display "auto_switch = false\n\n" port)
			       (display "[theme.custom]\n" port)
			       (display "accent = \"#89b4fa\"\n" port)
			       (display "panel_bg = \"#1e1e2e\"\n" port)
			       (display "surface0 = \"#313244\"\n" port)
			       (display "surface1 = \"#45475a\"\n" port)
			       (display "surface_dim = \"#181825\"\n" port)
			       (display "overlay0 = \"#6c7086\"\n" port)
			       (display "overlay1 = \"#7f849c\"\n" port)
			       (display "text = \"#cdd6f4\"\n" port)
			       (display "subtext0 = \"#a6adc8\"\n" port)
			       (display "mauve = \"#cba6f7\"\n" port)
			       (display "green = \"#a6e3a1\"\n" port)
			       (display "yellow = \"#f9e2af\"\n" port)
			       (display "red = \"#f38ba8\"\n" port)
			       (display "blue = \"#89b4fa\"\n" port)
			       (display "teal = \"#94e2d5\"\n" port)
			       (display "peach = \"#fab387\"\n\n" port)
			       (display "[keys]\n" port)
			       (display "navigate_pane_left = \"\"\n" port)
			       (display "navigate_pane_down = \"\"\n" port)
			       (display "navigate_pane_up = \"\"\n" port)
			       (display "navigate_pane_right = \"\"\n" port)
			       (display "focus_pane_left = \"shift+left\"\n" port)
			       (display "focus_pane_down = \"shift+down\"\n" port)
			       (display "focus_pane_up = \"shift+up\"\n" port)
			       (display "focus_pane_right = \"shift+right\"\n" port)))))))
