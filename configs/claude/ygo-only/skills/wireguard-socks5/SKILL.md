...
**split tunnel**: the system's default route is never touched, and neither
the tunnel nor the proxy start automatically on boot. General internet
traffic — including your own calls back to Anthropic — is completely
unaffected whether the tunnel is up or not.
...
Both services require an explicit start (they never auto-start):
...
Check status with `sudo herd status wireguard-wg0` / `socks5-proxy`.
